...
j = 0:d;                  % j è il vettore riga dei gradi da 0 a d 
...
if p(1) ~= 1
    p = [1; p(p~=1)]; %p(p~=1) seleziona tutti gli elementi di p che non sono uguali a 1.
                      %[1; p(p~=1)] concatena 1 come primo elemento (verticalmente, poiché viene usato ;), seguito dagli elementi filtrati.
end
