...
disponibile = true(M,1);   % Maschera logica: true se il punto NON è ancora stato scelto /spiega meglio
disponibile(1) = false;    % Il primo punto è già preso
prodvalori = ones(M,1);    % Vettore accumulatore per la produttoria: memorizza il prodotto delle distanze 
...
% L'obiettivo è trovare il prossimo punto xk che massimizza: ∏da j=1 a s-1 di|xk-ξj| dove ξj sono i punti di Leja già trovati
    ...
    dist(~disponibile) = 1;      % Imposta a 1 la distanza per i nodi già usati, evita di moltiplicare per 0
...
dist = [1,1,2]. (Corretto)
3) Aggiornamento Prodotto: prodvalori = [1,1,1]*[1,1,2]=[1,1,2].
4) Selezione: Oscuriamo i nodi non disponibili con -∞.
temp_prod = [-∞,1,2].
Il massimo è 2 (indice 3, valore x=1).
5) Risultato: dlp(2) = 1; disponibile(3) diventa false.
...
dist = [1,1,1]. (Corretto) 
Aggiornamento Prodotto: prodvalori precedente [1,1,2].
prodvalori = [1,1,2]*[1,1,1]=[1,1,2].
...
Il codice attuale usa una produttoria (prodvalori). 
...
% (parte precedente identica) prodvalori diventa sum_log_vals
