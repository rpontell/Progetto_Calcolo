...
- **[M,I] = max(A,[],___,"linear")** restituisce gli elementi massimi di un array. Se A è un vettore, max(A) restituisce il massimo di A, restituisce inoltre l'indice lineare in A che corrisponde al valore massimo in A. Se A è una matrice, max(A) è un vettore riga contenente il valore massimo di ciascuna colonna di A. Se A è un array multidimensionale, max(A) opera lungo la prima dimensione di A la cui grandezza non è uguale a 1, trattando gli elementi come vettori. La grandezza di M in questa dimensione diventa 1, mentre le grandezze di tutte le altre dimensioni rimangono le stesse come in A. Se A è un array vuoto la cui prima dimensione ha lunghezza pari a zero, M è un array vuoto con la stessa grandezza di A. Se A è una tabella o un orario, max(A) restituisce una tabella di una riga contenente il massimo di ciascuna variabile. (da R2023a)
- **ciclo For**
```matlab
  for index = values
     statements
...
- **mldivide, "\"** x = A\B risolve il sistema di equazioni lineari A* x = B. Le matrici A e B devono avere lo stesso numero di righe. visualizza un messaggio di avviso se A è mal condizionata o quasi singolare, ma esegue comunque il calcolo. Se A è uno scalare, allora A\B è equivalente a A.\B. Se A è una matrice quadrata n-by-n e B è una matrice con n righe, allora x = A\B è una soluzione all'equazione A* x = B, se esiste. Se A è una matrice rettangolare m-by-n con m ~= n, e B è una matrice con m righe, allora A\B restituisce una soluzione ai minimi quadrati al sistema di equazioni A* x= B. x potrebbe non essere la soluzione a norma minima.